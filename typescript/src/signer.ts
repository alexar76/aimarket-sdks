...
export class MarketSigner {
  ...
  signDebitAuthorization(params: DebitAuthorizationParams): string {
    ...
    // Defence in depth: parseEthPrivateKey already normalises the prefix in the
    // constructor, but hexToBytes REQUIRES it — handed a bare 64-char key it silently
    // drops two characters and returns 31 bytes, signing as a different wallet. If that
    // normalisation is ever bypassed, fail loudly here instead of emitting signatures
    // that look fine and are rejected on chain.
    const normalisedKey = (ethKey.startsWith('0x') ? ethKey : `0x${ethKey}`) as Hex;
    const privBytes = hexToBytes(normalisedKey);
    if (privBytes.length !== 32) {
      throw new Error(
        `ethereumPrivateKeyHex must be 32 bytes, got ${privBytes.length} — a truncated key ` +
          'signs as a different address and every signature is rejected on chain.'
      );
    }
    // prehash: false is load-bearing. @noble/curves v2 HASHES the message by default, so
    // omitting it signs sha256(digest) instead of the EIP-712 digest — self-consistent,
    // and rejected by AIMarketEscrow.debitChannel, which recovers over the digest.
    //
    // The 'recovered' layout is recovery‖r‖s (recovery FIRST), not r‖s‖recovery; reading it
    // the other way scrambles r, s and v together into a signature that recovers to noise.
    const recovered = secp256k1.sign(digestBytes, privBytes, {
      lowS: true,
      prehash: false,
      format: 'recovered',
    });
    const r = Buffer.from(recovered.slice(1, 33)).toString('hex');
    const s = Buffer.from(recovered.slice(33, 65)).toString('hex');
    // v is 27/28 — what Solidity's ECDSA.recover expects.
