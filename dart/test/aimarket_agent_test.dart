...
class _MockHttpClient extends http.BaseClient {
  ...
  /// Raw request bodies, in call order ('' for body-less requests).
  final List<String> bodyLog = [];
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    requestLog.add(key);
    bodyLog.add(request is http.Request ? request.body : '');
...
void main() {
  group('Capability', () {
    test('fromJson and toJson round-trip', () {
      ...
      expect(out['source_hub'], 'https://modelmarket.dev');
    ...
    test('withDefaults creates capability with sensible defaults', () {
      ...
      expect(cap.sourceHub, 'https://modelmarket.dev');
  ...
  group('InvokeResult', () {
    ...
    test('fromJson parses successful result', () {
      ...
      expect(result.result?['score'], 85);
      expect(result.priceUsd, 0.10);
      expect(result.teeVerified, true);
      expect(result.isSuccessful, true);
    });

    test('fromJson parses verification envelope', () {
      final result = InvokeResult.fromJson({
        'success': true,
        'result': {'ok': true},
        'price_usd': 0.10,
      ...
      expect(result.verification?['status'], 'pending');
      // toJson keeps the envelope under the snake_case wire key.
      expect(result.toJson()['verification'], result.verification);

      // Legacy hubs omit the field entirely — must stay null, not throw.
      final legacy = InvokeResult.fromJson({
        'success': true,
        'price_usd': 0,
        'latency_ms': 1.0,
      });
  ...
  group('SearchResponse', () {
    test('fromJson maps flat hub matches to plan steps', () {
      final json = {
        'query': 'fintech ATS',
        'matches': [
          {
      ...
      expect(resp.matches.length, 1);
      expect(resp.results.length, 1); // alias
      expect(resp.matches.first.capability.capabilityId, 'ats-rules-2026-q2');
      expect(resp.matches.first.capability.pricePerCallUsd, 0.10);
      expect(resp.matches.first.relevanceScore, 0.9);
      expect(resp.query, 'fintech ATS');
      expect(resp.catalog, 'factory');

      final out = resp.toJson();
      expect((out['matches'] as List).length, 1);
      expect(out['catalog'], 'factory');
    });

    test('defaults missing fields safely', () {
      final resp = SearchResponse.fromJson({});
      expect(resp.matches, isEmpty);
      expect(resp.query, '');
      expect(resp.catalog, '');
    });
  });
  ...
  group('MarketSigner', () {
    ...
    test('signCanonical produces deterministic signatures', () {
      final signer = MarketSigner(privateKeyHex: ed25519Seed);
    ...
    test('verify returns true for matching signature', () {
      ...
      expect(signer.verify(signer.publicKeyHex(), sig, 'test message'), true);
    });

    test('verify returns false for wrong message', () {
      final signer = MarketSigner(privateKeyHex: ed25519Seed);
      final sig = signer.signCanonical('message A');
      expect(signer.verify(signer.publicKeyHex(), sig, 'message B'), false);
    });
    ...
    test('signDebitAuthorization produces deterministic eip712 signature', () {
      final signer = MarketSigner(
        privateKeyHex: ed25519Seed,
        ethereumPrivateKeyHex: ethPrivateKey,
      );
      final args = {
        'channelId':
    ...
    test('verifyHubSignature verifies ed25519', () {
      ...
      expect(
        signer.verifyHubSignature(signer.publicKeyBase64(), 'hello hub', sig),
        true,
      );
    });
  ...
  group('AimarketAgent', () {
    ...
    test('openChannel returns a parsed channel', () async {
      ...
      mockClient.expectPost(
        'https://modelmarket.dev/ai-market/v2/channel/open',
        200,
    ...
    test('invoke sends the verify block in the body when set', () async {
      mockClient.expectPost(
        'https://modelmarket.dev/ai-market/v2/invoke',
        200,
        body: json.encode({
          ...
          'price_usd': 0.10,
          'latency_ms': 200,
          'verification': {'requested': true, 'status': 'pending'},
        }),
      );
      ...
      final result = await agent.invoke(
        ...
        input: {'role': 'PM'},
        channelId: 'ch_1',
        verify: {'requested': true, 'intent': 'score a PM resume'},
      );

      final sent = json.decode(mockClient.bodyLog.first) as Map<String, dynamic>;
      expect(sent['verify'],
          {'requested': true, 'intent': 'score a PM resume'});
      expect(result.verification?['status'], 'pending');
      agent.dispose();
    });

    test('invoke omits the verify block when not requested', () async {
      mockClient.expectPost(
        'https://modelmarket.dev/ai-market/v2/invoke',
        200,
        body: json.encode({
      ...
      final sent = json.decode(mockClient.bodyLog.first) as Map<String, dynamic>;
      expect(sent.containsKey('verify'), false);
    ...
    test('does NOT retry a verified-wait invoke (no double-charge)', () async {
      mockClient.expectPost(
        'https://modelmarket.dev/ai-market/v2/invoke',
        200,
        body: json.encode({
    ...
    test('getChannelBalance returns parsed balance', () async {
      mockClient.expectGet(
        'https://modelmarket.dev/ai-market/v2/channel/ch_1',
        200,
    ...
    test('closeChannel returns settlement', () async {
      mockClient.expectPost(
        'https://modelmarket.dev/ai-market/v2/channel/close',
        200,
        body: json.encode({
          'channel_id': 'ch_1',
