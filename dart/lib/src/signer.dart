...
import 'package:web3dart/crypto.dart' as crypto_utils;
...
class MarketSigner {
  ...
  String signDebitAuthorization({
    ...
    // secp256k1.sign() over the digest ITSELF, not EthPrivateKey.signToEcSignature():
    // that helper does `sign(keccak256(payload))`, so handing it a pre-computed EIP-712
    // digest signs keccak(digest). The result is perfectly self-consistent — it round-trips
    // through any recover() that repeats the same mistake — and AIMarketEscrow.debitChannel
    // rejects it as InvalidSignature(), because the contract recovers over the digest.
    //
    // `v` is 27/28 for the same reason. signToEcSignature(chainId: …) applies the EIP-155
    // transaction encoding (v - 27 + chainId*2 + 35), which is meaningless to Solidity's
    // ECDSA.recover; a chainId-encoded v is simply an invalid signature on chain.
    final ecSig = crypto_utils.sign(digest, creds.privateKey);
    final r = _bytesToHex(_bigIntToFixedBytes(ecSig.r, 32));
    final s = _bytesToHex(_bigIntToFixedBytes(ecSig.s, 32));
    final v = (ecSig.v < 27 ? ecSig.v + 27 : ecSig.v).toRadixString(16).padLeft(2, '0');
