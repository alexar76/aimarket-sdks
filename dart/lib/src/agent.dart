...
class AimarketAgentConfig {
  /// Base URL of the AI Market hub (e.g., "https://modelmarket.dev").
  final String hubUrl;
...
class AimarketAgent {
  ...
  /// channelId -> one-time debit secret, captured at open and sent on invoke.
  final Map<String, String> _channelSecrets = {};
  ...
  Future<List<PlanStep>> discover({
    ...
    // Hub returns { query, matches: [flat capability dicts], catalog }. Map each
    // flat match to a PlanStep — reading `results` (absent) always yielded [].
    final data = json.decode(resp.body) as Map<String, dynamic>;
    final items = data['matches'] as List<dynamic>? ?? [];
    return items
        .map((i) => PlanStep.fromSearchMatch(i as Map<String, dynamic>))
        .toList();
  }
  ...
  Future<Channel> openChannel(
    ...
    final channelMap = (decoded['channel'] as Map<String, dynamic>?) ?? decoded;
    final channel = Channel.fromJson(channelMap);
    ...
    final secret = channelMap['channel_secret'] as String?;
    if (secret != null && secret.isNotEmpty) {
      _channelSecrets[channel.id] = secret;
    }
  ...
  /// [verify] is the optional Pay-on-Verified opt-in, sent verbatim as the
  /// `verify` request block (`{'requested': true, 'intent': …, 'mode': …,
  /// 'wait': …, 'wait_timeout_s': …}`). The hub escrows the debit until Metis
  /// judges the output against the intent; the verdict envelope comes back as
  ...
  Future<InvokeResult> invoke({
    ...
    // A verified invoke with wait=true blocks server-side until Metis returns a
    // verdict while the hub holds the debit in escrow. Retrying it on a transient
    // error would re-POST a fresh nonce mid-wait and open a SECOND hold — the
    // buyer pays twice. Fire it exactly once (no retry) in that case.
    if (_verifyBlockWaits(verify)) {
      return _invokeOnce(
  ...
  bool _verifyBlockWaits(Map<String, dynamic>? verify) =>
      verify != null && verify['wait'] == true;

  /// Per-call timeout for a verified-wait invoke, or `null` when the invoke does
  /// not wait (use the config default). The hub holds the response until the
  /// Metis verdict, so allow `wait_timeout_s` (default 300) + 30s of slack —
  /// exceeding the wait bound keeps the call from timing out (then retrying and
  /// re-POSTing) mid-wait.
  Duration? _verifiedWaitTimeout(Map<String, dynamic>? verify) {
    if (!_verifyBlockWaits(verify)) return null;
    final secs = (verify!['wait_timeout_s'] as num?)?.toInt() ?? 300;
    return Duration(seconds: secs + 30);
  }
  ...
  Future<InvokeResult> _invokeOnce({
    ...
    final channelSecret = _channelSecrets[channelId];
    if (channelSecret != null && channelSecret.isNotEmpty) {
      headers['X-Payment-Channel-Secret'] = channelSecret;
    }
    ...
    // Verified-wait invokes get an extended per-call timeout past the hub's wait
    // bound so the request does not abort mid-wait — an abort would be retried
    // and re-POST a fresh nonce, creating a second hold/debit (buyer pays twice).
    final response = await _http
        ...
        .timeout(_verifiedWaitTimeout(verify) ?? _config.timeout);
