...
> **Two version lines, on purpose.** Dart / TypeScript / Rust are the lock-stepped cross-platform family at **0.2.x**; the Python agent is an older, separate package at **2.1.x** on PyPI. Both target AIMarket Protocol v2. See [`docs/sdk-version-policy.md`](../../docs/sdk-version-policy.md) — do not "fix" the mismatch.
